...
(function (Scratch) {
  ...
  // Load poly-decomp.js for converting concave polygons to convex polygons.
  // Trick the library we're running under CommonJS
  const module = {};
  const exports = {};
  
  // https://github.com/schteppe/poly-decomp.js/
  // (the library is MIT-licensed and this repo also is
  // so I don't think I need to include the full license text here?)
  /* eslint-disable */
  // prettier-ignore
  // @ts-ignore
  ...
  const _placeBody = function (id, x, y, dir, noFixtures) {
    if (bodies[id]) {
    ...
    body.uid = id;
    if (!noFixtures) body.CreateFixture(fixDef);
    bodies[id] = body;
    return body;
  };
  ...
  class Scratch3Griffpatch {
    ...
    setPhysicsFor(target, shape) {
      ...
      if (allHulls) {
        for (let i = 0; i < allHulls.length; i++) {
    ...
    _fetchPolygonPointsFromSVG(svg, hullPointsList, ox, oy, scaleX, scaleY) {
      ...
      const pushPoint = (point) => {
        if (hullPoints.length <= 0) return hullPoints.push(point);
        const last = hullPoints[hullPoints.length - 1];
        if (last[0] != point[0] || last[1] != point[1]) return hullPoints.push(point);
      }
      ...
      for (let i = 0; i < tokens.length; ) {
        ...
        if (
          ...
          for (let perc = dp; perc <= 1; perc += dp) {
            ...
            pushPoint([(px - ox) * scaleX, (py - oy) * scaleY]);
          }
        ...
        if (!addedPoint) pushPoint([(px - ox) * scaleX, (py - oy) * scaleY]);
      }
      ...
      if (hullPoints.length > 2) {
        decomp.makeCCW(hullPoints);
        const convexPolygons = decomp.quickDecomp(hullPoints);
        if (convexPolygons.length <= 0) convexPolygons.push(hullPoints);
        for (const polygon of convexPolygons) {
          // Reverse because Box2D apparently only properly handles clockwise polygons
          hullPointsList.push(polygon.map((arr) => ({x: arr[0], y: arr[1]})).reverse());
        }
      }
    }
